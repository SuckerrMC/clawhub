...
The final candidate passes **187 assertions** against actual local Convex queries/HTTP/storage and a built ClawHub Nitro server. It exercises103 ordinary HTTP requests,21 lifecycle HTTP requests,4 anonymous hash queries and4 preview identities. The owned runtime completed and cleaned up. No production fixtures or data were changed.
...
- After runtime tree: `04fe22d59e3d55efcec0992b442903c1be6ac6ae`, the exact final review candidate.
...
Additional parent controls: ordinary hidden-parent hash and file routes return 404; a parent flagged as malicious retains published hash metadata (200, match 1.0.0) while its file route remains 404. Focused before/after tests additionally prove removed/repointed tags are omitted even when the old version remains published.
